...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <>
      ...
      <aside
        className={`fixed lg:sticky top-0 left-0 z-50 h-full lg:h-screen w-72 bg-background-dark text-white flex flex-col transition-transform duration-300 shadow-2xl ${isOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"
          }`}
      >
        <div className="p-8 flex flex-col items-center border-b border-slate-800">
          <div className="relative group w-21.5 h-21.5 cursor-pointer overflow-hidden">
            <div className="w-full h-full rounded-full p-0.5 border border-white group-hover:border-brand-primary transition-colors">
              <Image
                src={ProfileData?.profile_image || "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&w=86&q=80"}
                alt="User"
                width={86}
                height={86}
          ...
          <h3 className="text-base font-semibold text-white">{`${ProfileData?.first_name ?? "Amanuel"}`}</h3>
          <p className="text-xs text-white font-normal mt-.5">
            {`${ProfileData?.email ?? "amanuel@gmail.com"}`}
          </p>
        </div>
        ...
        <nav className="flex-1 py-6 space-y-1">
          {menuItems.map((item) => (
            <Button
              key={item.label}
              variant="ghost"
              className={`w-full flex items-center gap-4 px-6 py-4 transition-all duration-200 justify-start rounded-none ${activeTab === item.label
